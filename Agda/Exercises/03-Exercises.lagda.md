...
well-ordering-principle P d (suc n) p = well-ordering-principle-suc P d n p (d 0) (well-ordering-principle (λ m → P (suc m)) (λ x → d (suc x)) n p)
```
...
is-zero-well-ordering-principle P d (suc m) pm = is-zero-well-ordering-principle-suc P d m pm (d 0) (well-ordering-principle (λ n → P (suc n)) (λ n → d (suc n)) m pm)
```
